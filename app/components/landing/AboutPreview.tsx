      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <AnimatedSection>
            ...
            <p className="text-text-2 text-lg leading-[1.85] mb-6">
              {fr
                ? "Créée par Donald Jean-Marie GNIMADI, entrepreneur culturel visionnaire originaire du Bénin, l'AAAM réunit des managers d'artistes, des promoteurs et des professionnels de la musique de tout le continent."
                : "Founded by Donald Jean-Marie GNIMADI, a visionary cultural entrepreneur from Benin, AAAM brings together artist managers, promoters and music professionals from across the continent."}
            </p>
            ...
            <Link href="/about" className="btn-outline px-6 py-3">
            ...
            <div className="grid grid-cols-2 gap-5">
              {[
                ...
                <div key={i} className="card p-6 text-center group">
                  <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-primary group-hover:text-white text-primary transition-all duration-200">
                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                    </svg>
                  </div>
                  <span className="text-text-1 font-semibold text-base">{item.label}</span>
                </div>
              ))}
            </div>
          </AnimatedSection>
